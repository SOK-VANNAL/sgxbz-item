...
import {LocalStorageService, QueryParam, SearchResult} from '@sgxbz/shared';
import {NzSelectComponent} from 'ng-zorro-antd/select';
import {BehaviorSubject} from 'rxjs';
import {debounceTime, map, switchMap} from 'rxjs/operators';
import { ItemList, ItemService } from './item.service';
import { ItemUiService } from './item-ui.service';
import {ItemTypeList} from '../item-type/item-type.service';
...
@Component({
  ...
  template: `
    <nz-select
      ...
      nzServerSearch
    >
      <nz-option *ngIf="!loading && allItemOption" [nzLabel]=" 'all_item' | translate" [nzValue]="-1"></nz-option>
...
export class ItemSelectComponent implements ControlValueAccessor, OnInit, OnDestroy{
  constructor(
  ...
  writeValue(value: any): void {
    ...
    if (!this.storageKey && this.val){
      this.itemService.find(this.val).subscribe(
        result => {
