...
import { Resource } from '../../share/resource/resource';
    ...
    <nz-layout>
      <nz-header style="display: flex; justify-content: space-between">
        <div style="width: 220px">
          <app-filter-input storageKey="unit-search" (filterChanged)="searchText=$event;onSearch()">
          </app-filter-input>
<!--          <app-unit-select storageKey="unit-select"></app-unit-select>-->
      ...
      <nz-content>
        <nz-table
          ...
          (nzPageSizeChange) = "localStorageService.setValue({key: unitPageSizeStorageKey, value: $event})"
          [nzData]="lists"
          [nzLoading]="loading"
...
export class UnitListComponent implements OnInit, OnDestroy{
  ...
  unitPageSizeStorageKey = 'unit-page-size';
  ...
  lists: UnitList[] = [];
  param: QueryParam = {
    pageSize: this.localStorageService.getValue(this.unitPageSizeStorageKey) ?? 10,
    pageIndex: 1,
    sorts: 'name'
  };
  ngOnInit(): void {
