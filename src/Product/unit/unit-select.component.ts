import {Component, forwardRef, HostBinding, Input, OnInit, ViewChild, Output, OnDestroy} from '@angular/core';
  ...
  template: `
    ...
    >
      ...
      <ng-template #actionItem>
        <a (click)="select.nzOpen = false; uiService.showAdd(componentId)" class="item-action">
...
export class UnitSelectComponent implements ControlValueAccessor, OnInit, OnDestroy{
  constructor(
  ...
  writeValue(value: any): void {
    ...
    if (!this.storageKey && this.val){
      this.unitService.find(this.val).subscribe(
        result => {
          if (result){
  ...
  ngOnInit(): void {
    ...
    if (this.storageKey){
      let recentSelected: UnitList[];
      // const recentSelected = this.unitService.getRecentSelect(this.storageKey);
  ...
  initSearch(): void{
    const searchResult = (name: string) =>
      ...
      }).pipe(map((res: SearchResult<UnitList>) => res.results));
  ...
  updateStorageKey(value: any): void {
    if (this.storageKey){
      ...
      // this.unitService.setRecentSelect(this.storageKey, selectedItem);
